...
func (app *application) routes() http.Handler {
	...
	//Protected routes
	mux.Handle("GET /snippet/create", app.sessionManager.LoadAndSave(preventCSRF(app.requireAuthentication(http.HandlerFunc(app.snippetCreate)))))
	mux.Handle("POST /snippet/create", app.sessionManager.LoadAndSave(preventCSRF(app.requireAuthentication(http.HandlerFunc(app.snippetCreatePost)))))
	mux.Handle("POST /user/logout", app.sessionManager.LoadAndSave(preventCSRF(app.requireAuthentication(http.HandlerFunc(app.userLogoutPost)))))
